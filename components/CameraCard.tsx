    ...
    <div className={s.card}>
      ...
      <div className={s.label}>
        ...
        <div className={s.actions}>
          {frame && (
            <button className={`${s.iconBtn} ${s.iconActive}`} onClick={() => setFrame(null)} title="Voltar ao vivo" aria-label="Voltar ao vivo">
              <Icon d={LIVE} />
            </button>
          )}
          <button
      ...
      {historyOpen && <HistoryStrip cam={cam.id} batch={historyBatch} intervalMin={Math.round(cam.intervalSec / 60)} selected={frame?.id ?? null} onOpen={setFrame} />}
    </div>
  );
}
